...
const SERPMockup = () => (
    <div className="w-full bg-white  border border-stone-200 p-5 flex flex-col shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        <div className="flex items-center justify-between mb-5 pb-4 border-b border-stone-100">
            ...
            <span className="text-[10px] font-medium bg-emerald-50 text-emerald-600 px-2.5 py-1 d border border-emerald-100">Live Tracking</span>
        </div>
            ...
            {[
                ...
                <div key={i} className="flex items-center justify-between py-2 border-b border-stone-50 last:border-0">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <span className={`text-[11px] font-bold w-6 h-6 d flex items-center justify-center flex-shrink-0 ${row.pos === 1 ? 'bg-emerald-500 text-white' : row.pos <= 3 ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-stone-100 text-stone-500'}`}>
                            {row.pos}
                        </span>
        ...
        <div className="flex items-end gap-2 h-24 mb-4">
            {[18, 24, 30, 28, 40, 52, 60, 68, 75, 80, 90, 100].map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div
                        className={`w-full sm transition-all ${i === 11 ? 'bg-emerald-500' : 'bg-stone-100'}`}
                        style={{ height: `${h}%` }}
                    ></div>
                </div>
            ))}
